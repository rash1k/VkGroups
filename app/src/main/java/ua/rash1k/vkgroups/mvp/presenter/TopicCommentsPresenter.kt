...
import ua.rash1k.vkgroups.rest.model.request.BoardGetCommentsRequestModel
import java.util.*
...
@InjectViewState
class TopicCommentsPresenter : BaseFeedPresenter<BaseFeedView>() {
    ...
    override fun onCreateLoadDataObservable(count: Int, offset: Int): Observable<BaseViewModel> {
        return mBoardApi.getComments(BoardGetCommentsRequestModel(
                mPlace.postId.toInt(), mPlace.ownerId.toInt(), count = count, offset = offset).toMap())
                .flatMap { full ->
                    Observable.fromIterable(getCommentList(full.response!!, true))
                }
                .doOnNext { it: CommentItem? -> it?.place = mPlace }
                .doOnNext(::saveToDb)
